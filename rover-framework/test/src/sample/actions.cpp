...
TEST(actions, play_to_expedition_success) {
	...
	player->m_hand.add_card(card(color::blue, 2));
	player->m_hand.add_card(card(color::blue, 3));
	EXPECT_EQ(player->m_hand.size(), 2);
	EXPECT_EQ(player->m_expeditions.total_score(), 0);
	state.execute(action::play_to_expedition(player->m_id, card(color::blue, 2)));
	EXPECT_EQ(player->m_hand.size(), 1);
	EXPECT_EQ(player->m_expeditions.total_score(), -18);
	state.execute(action::play_to_expedition(player->m_id, card(color::blue, 3)));
	EXPECT_EQ(player->m_hand.size(), 0);
	EXPECT_EQ(player->m_expeditions.total_score(), -15);
}
...
TEST(actions, play_to_expedition_wrong_card) {
	...
	EXPECT_EQ(player->m_hand.size(), 1);
	EXPECT_EQ(player->m_expeditions.total_score(), 0);
	// attempting to play a card that is not in the players hand should
	// result in the player discarding a random card
	state.execute(action::play_to_expedition(player->m_id, card(color::green, 8)));
	EXPECT_EQ(player->m_hand.size(), 0);
	EXPECT_EQ(player->m_expeditions.total_score(), 0);
...
TEST(actions, play_to_expedition_wrong_player_id) {
	...
	auto player = *state.player(player_1_id);
	player->m_hand.add_card(card(color::blue, 2));
	// attempting to execute an action with incorrect player id will trigger an assert
...
TEST(actions, play_to_expedition_ineligible_card) {
	...
	player->m_expeditions.add_card(card(color::blue, 3));
	state.execute(action::play_to_expedition(player->m_id, card(color::blue, 2)));
	// attempting to play a card, but it's ineligible due to a higher card 
	// already being the top card in that expedition should result in the player
...
TEST(actions, discard_card_success) {
	...
	EXPECT_EQ(player->m_hand.size(), 2);
	EXPECT_EQ(state.m_discard.size(color::blue), 0);
	state.execute(action::play_to_discard(player->m_id, card(color::blue, 2)));
	EXPECT_EQ(player->m_hand.size(), 1);
	EXPECT_EQ(state.m_discard.size(color::blue), 1);
...
TEST(actions, discard_card_wrong_card) {
	...
	// attempting to play a card that is not in the players hand should
	// result in the player discarding a random card
	state.execute(action::play_to_discard(player->m_id, card(color::green, 8)));
	EXPECT_EQ(player->m_hand.size(), 0);
	EXPECT_EQ(state.m_discard.size(color::blue), 1);
	EXPECT_EQ(state.m_discard.top(color::blue), card(color::blue, 2));
}
...
TEST(actions, draw_card_from_deck) {
	...
	auto player = *state.player(player_1_id);
	EXPECT_EQ(player->m_hand.size(), 0);
	state.execute(action::draw_from_deck(player->m_id));
	EXPECT_EQ(player->m_hand.size(), 1);
	EXPECT_TRUE(player->m_hand.contains(card(color::blue, 2)));
}
...
TEST(actions, draw_card_from_discard_success) {
	...
	constexpr auto player_2_id = 1;
	game_state state(player_1_id, player_2_id);
	state.m_discard.add_card(card(color::blue, 2));
	auto player = *state.player(player_1_id);
	EXPECT_EQ(player->m_hand.size(), 0);
	EXPECT_EQ(state.m_discard.size(color::blue), 1);
	state.execute(action::draw_from_discard(player->m_id, color::blue));
	EXPECT_EQ(player->m_hand.size(), 1);
	EXPECT_TRUE(player->m_hand.contains(card(color::blue, 2)));
	EXPECT_EQ(state.m_discard.size(color::blue), 0);
}
...
TEST(actions, draw_card_from_discard_failiure) {
	...
	// result in the player drawing form the deck instead
	state.execute(action::draw_from_discard(player->m_id, color::green));
	EXPECT_EQ(player->m_hand.size(), 1);
	EXPECT_TRUE(player->m_hand.contains(card(color::blue, 2)));
}